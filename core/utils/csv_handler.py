...
def generate_files(file):
    try:
        ...
        print(
            f"[Debug] Error in generate_files() at line {e.__traceback__.tb_lineno}: {e}"
        )
        raise
...
def process_csv_file(init_test_time, end_test_time):
    try:
        ...
        time_intervals = [
            t for t in union_time() if init_test_time <= t <= end_test_time
        ]
        ...
        for i in range(n_intervals):
            ...
            row = {
                ...
                "Mobile_Radio(ON/OFF)": look_up_bool(
                    "Mobile radio active.csv", start_time, end_time
                ),
        ...
        csv_filename = os.path.join(
            OUTPUT_DIR, f"PowDroid_{datetime.now().timestamp()}.csv"
        )
        ...
        print(
            f"[Debug] Error in process_csv_file() at line {e.__traceback__.tb_lineno}: {e}"
        )
        raise
